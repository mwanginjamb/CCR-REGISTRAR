...
<body class="bg-background text-on-surface">
    ...
    <!-- ============================================================
     ...
     ============================================================ -->
    <aside
        class="nav-aside fixed left-0 top-0 h-full flex flex-col p-4 z-40 bg-[#eceef0] dark:bg-slate-800 w-64 shadow-[12px_0_32px_rgba(0,26,72,0.04)] transition-all duration-300 transition-transform">

        <!-- Logo -->
        <div class="mb-10 lg:px-4 flex justify-center lg:justify-start">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 bg-primary-container rounded-lg flex items-center justify-center text-white shrink-0">
                    <span class="material-symbols-outlined">biotech</span>
                </div>
                <div class="logo-text">
                    <h1 class="text-lg font-bold text-[#001a48] dark:text-blue-100 headline-font leading-tight">Clinical
                        Curator</h1>
    ...
    <main class="main-canvas ml-0 lg:ml-64 min-h-screen transition-all duration-300">
        ...
        <!-- Top Navigation Bar -->
        <header
            class="flex justify-between items-center w-full px-4 lg:px-8 h-16 sticky top-0 z-30 bg-[#f2f4f6] dark:bg-slate-900 border-b-0">
            <div class="flex items-center gap-4 lg:gap-8">

                <!-- Sidebar Toggle -->
                <button
                    class="w-10 h-10 flex items-center justify-center text-slate-500 hover:bg-[#eceef0] rounded-full transition-colors z-50"
                ...
                <div class="relative hidden sm:block">
                    ...
                    <input
                        class="bg-surface-container-highest border-none rounded-full pl-10 pr-4 py-1.5 text-sm w-48 md:w-64 focus:ring-2 focus:ring-primary-container"
                        placeholder="Search registry..." type="text" />
                </div>
            </div>

            <div class="flex items-center gap-2 lg:gap-4">
                <button
                    class="w-10 h-10 flex items-center justify-center text-slate-500 hover:bg-[#eceef0] rounded-full transition-colors"
                    aria-label="Notifications">
                    <span class="material-symbols-outlined">notifications</span>
                </button>
                <button
                    class="hidden md:flex w-10 h-10 items-center justify-center text-slate-500 hover:bg-[#eceef0] rounded-full transition-colors"
                    aria-label="Help">
        ...
        <?php if (Yii::$app->session->hasFlash('success')): ?>
            <div
                class="mx-4 lg:mx-8 mt-4 p-4 bg-secondary-container/20 border border-secondary/20 rounded-xl flex items-center gap-3">
        ...
        <?php if (Yii::$app->session->hasFlash('error')): ?>
            <div
                class="mx-4 lg:mx-8 mt-4 p-4 bg-error-container/40 border border-error/20 rounded-xl flex items-center gap-3">
    ...
    <!-- Contextual FAB -->
    <button
        class="fixed bottom-6 right-6 lg:bottom-8 lg:right-8 w-14 h-14 bg-primary text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50"
